...
1. 定义预设、方案、关联、节点、关系和导出文件的纯领域 Schema 与校验器。
2. 建立 IndexedDB 仓储、迁移标记和旧已有帕鲁导入。
3. 实现预设选择面板、显式保存及独立预设管理。
4. 实现独立方案管理、空画布、自动保存和多对多关联。
5. 实现节点添加、创建子代、删除、合并、自动布局和文本关系列表。
6. 将“子代反查亲本”改为“获取目标帕鲁”，接通追加到当前方案和节点跳转。
7. 完成方案 JSON 导入导出、错误恢复、键盘/触屏替代操作和响应式回归。
8. 移除旧路径规划 UI、Worker、代数设置和无消费代码，更新产品与架构文档。
