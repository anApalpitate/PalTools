...
platforms: [electron, windows, node]
source_of_truth: [script/electron/main.cjs, script/package-exe.ps1, package.json]
...
生产窗口使用 `contextIsolation: true`、`nodeIntegration: false` 和沙箱。`paltools://` 自定义协议只解析 `build/web` 下的规范化路径，拒绝越界路径；HTTP(S) 链接只能交给系统外部浏览器，窗口内导航被阻止。只有隔离用户目录的打包 smoke 模式为自动化兼容临时关闭沙箱。

## 后果

- 前端不能依赖 Electron 或 Node API，Web 与桌面共用同一静态构建。
- 自定义协议、安全开关、导航处理和 smoke 断言属于高风险模块，修改后必须执行正式 `package:exe` 发布门。
