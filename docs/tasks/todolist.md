...
id: tasks-backlog
title: PalTools 待办
summary: 下一阶段仅优化现有功能的可靠性、性能与维护成本，共七项待办。
type: task
status: current
...
本阶段由用户明确限定为现有功能优化。下列七项均为待实施，不新增页面、业务能力、模型厂商、检索来源、账号、云同步或数据格式；不引入通用状态框架、数据库替换、向量服务或大规模目录迁移。性能项先记录基线，只有收益可测且行为等价才保留实现。
...
1. 第一批：OPT-01 → OPT-02 → OPT-03，先处理部分写入、并发覆盖与错误恢复。
2. 第二批：OPT-04 → OPT-05；OPT-06 可独立测量。可靠性测试先固定行为，再调整读取与渲染结构。
3. 第三批：OPT-07，按测得的重复成本选择最小修改，不为节省构建时间绕过验证。
...
本次仅更新 TODO 与任务索引。README、文档首页和当前参考文档已审查，未发生需要写入 canonical 页面的实现变化；新增计划只保留在本页，前一批完成记录保持原归档。
