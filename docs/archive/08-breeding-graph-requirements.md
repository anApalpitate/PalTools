...
本文记录已有帕鲁预设、获取目标帕鲁和可编辑配种图的完整阶段需求及验收结果。实现未改变静态帕鲁 Schema、无性别配方数据、图鉴搜索规则或 Electron 安全配置；稳定行为已并入产品与架构参考文档。
...
| 2（已完成，2026-08-01） | 完成预设与方案资源管理 | 预设选择面板、独立方案管理、多对多关联与未保存保护 |
| 3（已完成，2026-08-01） | 建立可持久化的配种森林核心 | 节点、配种关系、受控画布、自动布局和文本关系列表 |
| 4（已完成，2026-08-01） | 完成编辑工具与目标查询闭环 | 完整工具栏、配方追加、节点跳转和跨页面上下文 |
...
实施结果：预设选择、统一搜索、全选结果、清空已选和显式保存已落地；预设与方案均可独立创建、切换、重命名和删除。当前方案会显示全部关联预设，并提供显式关联和解除入口，解除关联不会删除资源或图内容。预设草稿在切换预设、切换方案、离开配种页和关闭窗口时均受保护，保存失败会保留草稿、显示错误并阻止继续离开。方案画布内容仍为空，真实节点保存状态从阶段 3 开始接入。

验证记录：配种图工作区、配种页、应用壳与工作区 Hook 共 4 个定点测试文件、22 项测试通过；完整测试 19 个文件、155 项通过，TypeScript、300 个帕鲁/44,851 条无性别配方的数据校验和生产构建均通过。真实 Chromium 验证了预设选择、全局未保存保护、显式保存和预设—方案关联；1440×900、1152×720、1366×768 与 390×844 均无页面级横向溢出或破图，控制台无警告/错误，运行时请求均来自本地预览。阶段 2 不执行 EXE 打包，桌面发布 smoke 留在阶段 5。
...
实施结果：新增带 `recipeIndex` 的精确配方匹配接口和独立方案编辑 Hook。当前预设队列支持拖入以及键盘/触屏可用的“添加到画布”，同一帕鲁可创建多个节点实例；受控 React Flow 画布支持选择、移动、平移、缩放和适应画布。选择两个不同节点后可创建唯一子代，多结果时显式选择，无结果不修改方案；每条领域关系保存一个精确配方索引并渲染两条父子边。内置确定性分层布局保持亲本在上、子代在下，文本区同步提供等价配种公式。节点、关系、坐标、视口及预设关联使用 IndexedDB v1 原子保存，500ms 防抖自动保存，切换方案或内部导航前强制刷新，失败时保留草稿并可重试。

验证记录：阶段 3 定点 6 个文件、28 项测试通过；完整测试 21 个文件、161 项通过，TypeScript、300 个帕鲁/44,851 条无性别配方的数据校验和生产构建均通过。真实 Chromium 创建了同帕鲁重复节点、第二亲本和子代关系，得到 4 个节点、2 条可视边和 1 条等价文本关系；刷新后全部恢复。1440×900、1152×720、1366×768 与 390×844 均无页面级横向溢出或破图，控制台无警告/错误，运行时请求均来自本地预览。阶段 3 不执行 EXE 打包。
...
### 阶段 4：完整编辑工具与目标查询闭环（已完成）

实施结果：画布补齐选择/移动、平移、查询获取方式、创建子代、同帕鲁合并、删除、撤销/重做、自动整理、适应画布、缩放和加入当前预设工具。撤销/重做按方案保留最多 100 个当前会话内存快照；合并和删除先生成候选方案并通过完整领域校验。正向精确双亲、单亲展开与“获取目标帕鲁”全部使用带 `recipeIndex` 的配方匹配结果，可向当前方案追加独立的两个亲本节点、一个子代节点和一条关系。节点查询会保存方案、预设和视口上下文，追加或返回后继续编辑原方案；新子代加入预设只修改预设草稿，仍需显式保存。

验证记录：阶段 4 定点 5 个文件、31 项测试通过；完整测试 21 个文件、165 项通过，TypeScript、300 个帕鲁/44,851 条无性别配方的数据校验和生产构建均通过。真实 Chromium 验证了单亲追加、目标反查追加、6 个节点/4 条可视边/2 条文本关系的自动保存、撤销重做、节点查询返回和删除影响提示。1440×900、1152×720、1366×768 与 390×844 均无页面级横向溢出，可见区域无破图；干净生产构建会话无控制台警告/错误，运行时请求均来自本地预览。阶段 4 不执行 EXE 打包。
...
### 阶段 5：导入导出、可访问性与发布验收（已完成）

实施结果：新增 `.paltools-plan.json` 整体导入导出。导入限制为 5 MiB、1,000 个节点和 1,000 条关系；解析后重建全部方案、节点和关系 ID，使用不冲突方案名，将节点来源统一标记为 `import`，不导入或绑定预设，也不覆盖现有方案。数据集版本不一致时先显式提示，再按亲本和子代身份使用当前配方索引重映射 `recipeIndex`；任何 Schema、帕鲁、配方、DAG 或上限校验失败均在写入前整体拒绝。仓储使用单一 IndexedDB v1 事务写入导入方案和工作区元数据，无需升级数据库。

可访问性与性能收口：预设网格保留键盘/触屏“添加到画布”替代操作；画布节点可通过 Enter/Space 选择，弹窗支持初始聚焦和 Escape，状态变化通过可访问消息反馈。八套主题下的配种图表面、控件和画布均消费语义令牌。300 项预设过滤实测约 25.5ms；500 节点导入并渲染约 70.9ms，确定性自动整理与界面更新约 449.4ms，满足 500ms 目标。

验证记录：阶段 5 定点 6 个文件、33 项测试通过；最终完整测试 22 个文件、174 项通过，TypeScript、300 个帕鲁/44,851 条无性别配方的数据校验和生产构建均成功。真实 Chromium 覆盖八套主题、1440×900、1152×720、1366×768、1024×600 和 390×844，验证了导出、等价独立导入、同名避让、版本差异提示、零预设关联、键盘添加与节点选择、500 节点渲染和自动保存；未发现页面级横向溢出、可见破图、控制台警告/错误或第三方运行时请求。最终 `package:exe` 通过 electron-builder 和打包应用 schema、反向索引、主动技能、掉落图标及主题设置 smoke；产物 `PalTools-0.1.0-win-x64.exe` 为 87,718,822 字节（83.7 MB），SHA-256 为 `448E539E573436EDB26E71C8AB9A454C110F4370018E4B51B107061E884FE3A4`。
