...
- 方案网桌面使用独立配方背包侧栏，窄屏改为带焦点圈定、Escape 关闭和焦点恢复的抽屉。步骤、图形和关系列表共享同一派生关系集合；背包项把本地头像横向排列、名称置于头像下方，并在右下元信息行显示方案状态与配方编号。背包工具栏固定按全选、已加入清单过滤、默认开启的疾旋鼬自交过滤、排序字段和方向排列。步骤及关系列表共享角色卡片公式，使用放大头像与非颜色角色标记。
