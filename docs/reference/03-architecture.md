...
- 配种页以三个标签切换双亲查子代、获取目标帕鲁和配种方案网。查询卡共享即时背包状态，页面切换不清空查询或收藏；两类查询共享传说过滤、编号/平均稀有度及正倒方向排序和图形化稀有度展示，传说视觉只消费主题警示令牌。查询卡收藏使用带悬停说明和可访问名称的紧凑 `＋/✓` 图标。
- 方案网桌面使用独立关系背包侧栏，窄屏改为带焦点圈定、Escape 关闭和焦点恢复的抽屉。步骤、图形和关系列表共享同一派生关系集合；背包行、步骤卡和关系列表复用带本地头像的紧凑亲本 A + 亲本 B → 子代表示。
- 图形网使用只读 React Flow；领域层不生成配方操作节点：合并模式由帕鲁节点和带配方编号/亲本角色的直连边组成，实例模式由亲本/子代实例和同种帕鲁汇合点组成。所有帕鲁节点显示本地头像，移除关系的操作附在唯一一条配方边标签上。ELK layered 布局通过独立 Worker 执行，使用稳定输入、固定从左到右选项和整数坐标。递增请求 ID 丢弃过期响应，React 不持久化坐标。React Flow 的节点、边、标签、连接点、背景和控制按钮通过其 CSS 变量映射到 PalTools 主题令牌，不使用库默认调色板。
