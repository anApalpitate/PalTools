...
- 视口平移和滚轮缩放通过 `requestAnimationFrame` 直接更新变换层，交互结束后才提交钳制到 `0.2–1.5` 的最终视口；节点和连线按可见区域加 overscan 裁剪。配种图进入时由应用壳锁定顶栏以下的剩余视口；“加入帕鲁”采用覆盖式侧栏，关闭后不保留布局列，文本关系列表为独立可折叠面板。
