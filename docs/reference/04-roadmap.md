...
REQ-003 已交付关系背包、默认与自定义方案、DAG 循环约束、失效关系保留、稳定文本步骤、React Flow/ELK Worker 图形网、虚拟化关系列表、规范化 IndexedDB 与完整工作区导入导出。查询入口、方案页和桌面 smoke 共用同一 `recipeIndex` 身份边界；旧手工配种图数据库仍只删除、不迁移。
