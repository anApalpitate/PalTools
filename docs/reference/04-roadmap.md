...
已交付运行时混合知识检索、八个只读工具、四种模型协议、流式工具循环、安全桌面网关、Web 会话密钥、版本化对话档案，以及带证据索引脊线的三栏研究终端。助手联网需用户自备模型服务并首次确认数据披露；原有图鉴、配种和方案工作区仍保持离线完整。当前行为见[产品需求](01-product-requirements.md)、[架构说明](03-architecture.md)和[本地证据 Agent 决策](../decisions/0004-local-evidence-agent.md)。

## 阶段 8：架构与文件编排收敛（已完成）

已补齐 Node TypeScript 与 CJS/MJS 检查入口，统一 Web/CLI 静态数据 envelope 契约和可恢复加载状态，将知识存储契约与检索实现分离，并让 Web/Electron 共用唯一 Provider 协议实现。图鉴详情、正反向查询、关系背包及配种样式按职责拆分，焦点圈定与滚动锁收拢为有限共享 hook；多视口、七主题、离线检索、弹窗/抽屉和 Worker 图形网现可由单一浏览器回归命令重复验证。当前边界与命令见[架构说明](03-architecture.md)和[快捷命令](07-quick-commands.md)。

## 当前阶段

当前没有已确认但尚未实施的产品开发需求。新增能力先进入 Todolist 并完成需求确认，再写入实施路线图。
