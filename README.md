...
“帕鲁研究终端”会先检索应用内的图鉴、技能、词条、掉落与配种索引，再把必要证据交给你选择的模型组织答案。回答旁的档案编号可展开实际检索记录，并直接跳回图鉴或配种页面。输入 `@` 还能显式选择本地工具，或引用帕鲁、主动技能和掉落物；这些调用始终先在本机执行。
...
- 对话区使用主题纯色工作面；`Enter` 换行，`Ctrl/⌘/Shift + Enter` 发送。
