...
- 内置 OpenAI、Anthropic、Gemini、DeepSeek、通义千问（中国内地）、Kimi、OpenRouter、Ollama 与自定义接口模板；其他兼容服务可通过自定义接口填写。
...
- 模型服务可直接在发送框内切换，并随当前研究记录保存；对话区使用主题纯色工作面，快捷键提示只在输入框聚焦时出现，`Enter` 换行，`Ctrl/⌘/Shift + Enter` 发送。
