...
“帕鲁研究终端”会先检索应用内的图鉴、技能、词条、掉落与配种索引，再把必要证据交给你选择的模型组织答案。回答旁的档案编号可展开实际检索记录，并直接跳回图鉴或配种页面。输入 `@` 可以直接选择带本地缩略图或分类标记的帕鲁、主动技能和掉落物：单只帕鲁默认读取完整资料，2–4 只帕鲁默认比较，技能与物品分别反查拥有者和掉落来源；问题中写明配种等意图时会自动改用对应的只读查询。

- 内置 OpenAI、Anthropic、Gemini、DeepSeek、智谱 GLM（中国内地）、通义千问（中国内地）、Kimi、OpenRouter、Ollama 与自定义接口模板。每个模板预设少量常用模型，一次配置服务商密钥后即可管理显示型号、添加自定义 ID，并为每个型号单独设置参数。
...
- 尚未保存模型服务时，助手只显示配置引导页；完成“设置 → 模型服务”配置后才显示对话、本地资料引用与回答依据。删除全部配置会回到引导页，已有研究记录继续保留。
- 模型选择器按服务连接分组，只显示已启用型号；选择结果以“服务连接＋模型 ID”随当前研究记录保存，修改连接默认型号只影响新记录。对话区使用主题纯色工作面，快捷键提示只在输入框聚焦时出现，`Enter` 换行，`Ctrl/⌘/Shift + Enter` 发送。
...
5. 如需智能问答，在“设置 → 模型服务”选择服务商、填写 API Key 和默认模型；需要时再展开模型与连接高级设置。
