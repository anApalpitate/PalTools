...
describe('breeding workspace domain', () => {
  ...
  it('derives stable components, topological steps and equivalent graph modes', () => {
    ...
    expect(merged.nodes.filter((node) => node.kind === 'pal')).toHaveLength(5)
    expect(merged.nodes.filter((node) => node.kind === 'recipeJunction')).toHaveLength(2)
    expect(merged.edges).toHaveLength(6)
    expect(merged.edges.filter((edge) => edge.role === 'parentInput')).toHaveLength(4)
    expect(merged.edges.filter((edge) => edge.role === 'offspringOutput')).toHaveLength(2)
    expect(instance.nodes).toHaveLength(13)
    expect(instance.nodes.some((node) => node.kind === 'speciesJunction')).toBe(true)
    expect(instance.nodes.some((node) => node.kind === 'recipeJunction')).toBe(true)
